...
public class AttributeRouteCollector implements RouteCollectorContract {
        ...
        for (Class<?> clazz : classes) {
            for (Method method : clazz.getMethods()) {
                ...
                for (Route routeAnnotation : routeAnnotations) {
                    io.valkyrja.cli.routing.data.Route route =
                            convertAnnotationToData(routeAnnotation, clazz, method);
    ...
    protected io.valkyrja.cli.routing.data.Route convertAnnotationToData(
        ...
        return new io.valkyrja.cli.routing.data.Route(
                annotation.name(), annotation.description(), handler);
    }

    protected io.valkyrja.cli.routing.data.Route updateName(
            io.valkyrja.cli.routing.data.Route route, Class<?> clazz, Method method) {
        ...
        if (classNameAnnotation != null) {
            route =
                    (io.valkyrja.cli.routing.data.Route)
                            route.withName(classNameAnnotation.value() + "." + route.getName());
        }
        if (methodNameAnnotation != null) {
            route =
                    (io.valkyrja.cli.routing.data.Route)
                            route.withName(route.getName() + "." + methodNameAnnotation.value());
        }
    ...
    protected io.valkyrja.cli.routing.data.Route updateMiddleware(
            io.valkyrja.cli.routing.data.Route route, Method method) {
        ...
        for (Middleware m : middlewares) {
            ...
            if (RouteMatchedMiddlewareContract.class.isAssignableFrom(middlewareClass)) {
                ...
                route =
                        (io.valkyrja.cli.routing.data.Route)
                                route.withAddedRouteMatchedMiddleware(list);
            }
            ...
            if (RouteDispatchedMiddlewareContract.class.isAssignableFrom(middlewareClass)) {
                ...
                route =
                        (io.valkyrja.cli.routing.data.Route)
                                route.withAddedRouteDispatchedMiddleware(list);
            }
    ...
    protected io.valkyrja.cli.routing.data.Route updateArguments(
            io.valkyrja.cli.routing.data.Route route, Method method) {
        ...
        return (io.valkyrja.cli.routing.data.Route)
                route.withArguments(params.toArray(new ArgumentParameterContract[0]));
    }

    protected io.valkyrja.cli.routing.data.Route updateOptions(
            io.valkyrja.cli.routing.data.Route route, Method method) {
        ...
        for (OptionParameter o : annotations) {
            params.add(
                    new io.valkyrja.cli.routing.data.OptionParameter(
                            ...
                            o.valueDisplayName(),
                            o.defaultValue(),
                            Arrays.asList(o.shortNames()),
                            Arrays.asList(o.validValues()),
                            new ArrayList<>(),
                            o.mode(),
                            o.valueMode()));
        }
        ...
        return (io.valkyrja.cli.routing.data.Route)
                route.withOptions(params.toArray(new OptionParameterContract[0]));
    }
}
