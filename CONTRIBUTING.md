...
Java code, tests or documentation.
...
4. PRs must pass all CI checks. Please run all the CI checks locally.
    1. Spotless (formatting): `./gradlew spotlessCheck`
       Run `./gradlew spotlessApply` to auto-fix formatting before submitting.
    2. ArchUnit (architecture): `./gradlew archunit`
    3. Error Prone (static analysis): `./gradlew errorprone`
    4. SpotBugs (bug detection): `./gradlew spotbugs`
    5. JUnit (unit tests): `./gradlew junit`
       Or run all at once: `./gradlew ci`
...
The overall directory structure is set by rules within ArchUnit.
...
[issues url]: https://github.com/valkyrjaio/valkyrja-java/issues
